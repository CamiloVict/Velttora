...
const REASON_TEMPLATES = {
  ...
  government: {
    ...
    admin: {
      ...
      preheader: (d) =>
        `${d.name} from a public or institutional body reached out`,
      intro:
    ...
    user: {
      ...
      preheader:
        "We received your institutional inquiry and will respond shortly.",
  ...
  builder: {
    ...
    user: {
      ...
      preheader:
        "Your message reached the Velttora team — we’ll get back to you.",
      ...
      closing: "Keep building — we’re glad you said hello.",
    },
  },
  ...
  other: {
    ...
    admin: {
      ...
      intro:
        "A general inquiry was submitted through the website contact form.",
      subject: (d) =>
        `[Velttora] Contact — ${d.name}${
          d.organization ? ` · ${d.organization}` : ""
        }`,
    },
    user: {
...
<body style="margin:0;padding:0;background:${
  ...
  <span style="display:none;max-height:0;overflow:hidden;">${escapeHtml(
    preheader,
  )}</span>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:${
          ...
          <tr>
            <td style="padding:32px 32px 24px;border-bottom:1px solid rgba(255,255,255,0.08);">
              <p style="margin:0;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:${
                BRAND.accent
              };">Velttora LLC</p>
              <h1 style="margin:12px 0 0;font-size:24px;line-height:1.3;color:${
                BRAND.text
              };font-weight:700;">${escapeHtml(title)}</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 32px 32px;color:${
              BRAND.muted
          ...
          <tr>
            <td style="padding:20px 32px 28px;border-top:1px solid rgba(255,255,255,0.08);font-size:12px;color:#55556a;line-height:1.6;">
              <a href="${BRAND.siteUrl}" style="color:${
    BRAND.accent
  };text-decoration:none;">velttora.com</a>
...
function fieldRow(label, value) {
  return `<tr>
    <td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);vertical-align:top;">
      <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#55556a;">${escapeHtml(
        label,
      )}</p>
      <p style="margin:0;color:${
        BRAND.text
      };font-size:15px;line-height:1.5;">${escapeHtml(value)}</p>
    </td>
  </tr>`;
}

function reasonBadge(reasonLabel) {
  return `<span style="display:inline-block;background:rgba(0,229,160,0.12);border:1px solid rgba(0,229,160,0.25);color:${
    BRAND.accent
  };font-size:11px;letter-spacing:0.08em;text-transform:uppercase;padding:6px 12px;border-radius:4px;margin-bottom:16px;">${escapeHtml(
    reasonLabel,
  )}</span>`;
}
...
export function adminNotificationEmail(data) {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;color:${BRAND.muted};">${escapeHtml(
    tpl.admin.intro,
  )}</p>
    ...
    <p style="margin:0;">
      <a href="mailto:${escapeHtml(
        data.email,
      )}" style="display:inline-block;background:${BRAND.accent};color:${
    BRAND.bg
  };font-weight:600;text-decoration:none;padding:12px 22px;border-radius:6px;font-size:14px;">Reply to ${escapeHtml(
    data.name,
  )}</a>
    </p>`;
...
export function userConfirmationEmail(data) {
  ...
  const stepsHtml = tpl.user.nextSteps
    .map(
      (step) =>
        `<li style="margin-bottom:8px;">${interpolate(step, data)}</li>`,
    )
    .join("");

  const body = `
    <p style="margin:0 0 16px;color:${
      BRAND.text
    };font-size:17px;">Hi ${escapeHtml(data.name)},</p>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:rgba(0,229,160,0.08);border:1px solid rgba(0,229,160,0.2);border-radius:8px;margin-bottom:24px;">
      <tr>
        <td style="padding:20px 22px;">
          <p style="margin:0 0 12px;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:${
            BRAND.accent
          };">Your ${escapeHtml(tpl.label)} inquiry</p>
          <p style="margin:0 0 8px;color:${
            BRAND.text
          };"><strong>Type:</strong> ${escapeHtml(data.reason || tpl.label)}</p>
          ${
            data.organization
              ? `<p style="margin:0 0 8px;color:${
                  BRAND.text
                };"><strong>Organization:</strong> ${escapeHtml(
                  data.organization,
                )}</p>`
              : ""
          }
          <p style="margin:0;color:${
            BRAND.muted
          };font-size:14px;line-height:1.6;">${escapeHtml(data.message)}</p>
        </td>
      </tr>
    </table>
    <p style="margin:0 0 12px;"><strong style="color:${
      BRAND.text
    };">What happens next?</strong></p>
    <ul style="margin:0 0 24px;padding-left:20px;color:${BRAND.muted};">
      ${stepsHtml}
    </ul>
    <p style="margin:0;color:${BRAND.muted};font-size:14px;">${interpolate(
    tpl.user.closing,
    data,
  )}</p>`;
